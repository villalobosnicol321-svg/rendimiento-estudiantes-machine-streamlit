...
# Nombres en inglés (label visible) → clave interna
OPCIONES_MODELO = {
...
# Agrupación de variables del formulario
CAMPOS_ACADEMICO = ["Hours_Studied", "Attendance", "Previous_Scores", "Tutoring_Sessions"]
CAMPOS_HABITOS = ["Sleep_Hours", "Physical_Activity", "Extracurricular_Activities", "Motivation_Level"]
CAMPOS_SOCIO = [
...
with st.sidebar:
    ...
    st.metric(
        ...
        help="F1 Macro averages the F1 score of each class (Deficient, Basic, Superior) giving equal weight to all, even if one class has fewer students. Closer to 1.0 is better.",
    )
    st.metric(
        "Accuracy",
        f"{info_sel['accuracy']:.3f}",
        help="Percentage of students the model classified correctly. E.g. 0.939 = 93.9% correct. High accuracy can be misleading if one class dominates the data.",
    )
    st.metric(
        "AUC-ROC",
        f"{info_sel['auc_roc']:.3f}",
        help="Measures how well the model separates the 3 levels. 1.0 = perfect separation, 0.5 = no better than random. Values above 0.85 are considered very good.",
    )
    ...
    with st.expander("📖 Metrics glossary"):
        st.markdown("""
**F1 Macro**
Balances precision and recall across all 3 performance levels equally. Best metric when classes are unbalanced (few Superior students).

**Accuracy**
How many students were classified correctly out of the total. Simple but can be misleading — a model that always predicts "Basic" would score ~83% since most students are Basic.

**AUC-ROC**
Measures the model's ability to rank students correctly across levels. 0.9+ is excellent; 0.7–0.9 is good; below 0.7 needs improvement.

**Confidence**
Probability the selected model assigns to its own prediction. E.g. 99.3% means the model is very certain about its output.
        """)
...
with st.form("formulario_estudiante"):
    ...
    valores_en = {}
    campos_num_academico = [c for c in CAMPOS_ACADEMICO if c in metadata["features_num"]]
    campos_num_habitos = [c for c in CAMPOS_HABITOS if c in metadata["features_num"]]

    with col1:
        for col in campos_num_academico:
            cfg = opciones[col]
    ...
    campos_socio_cat = [c for c in CAMPOS_SOCIO if c in metadata["features_cat"]]
    mitad = len(campos_socio_cat) // 2

    with col3:
        for col in campos_socio_cat[:mitad]:
            cfg = opciones[col]
            opciones_es = [valor_a_es(v) for v in cfg["opciones"]]
            mapa = {valor_a_es(v): v for v in cfg["opciones"]}
            default_es = valor_a_es(cfg["default"])
    ...
    # Any remaining numeric fields not yet captured
    for col in metadata["features_num"]:
        if col not in valores_en:
            cfg = opciones[col]
            valores_en[col] = int(cfg["default"])
